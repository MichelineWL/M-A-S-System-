...
import { Send, User, Bot, Loader2, BarChart2, Sparkles, CheckCircle2, Brain, Zap, Users } from 'lucide-react';
...
export default function ChatInterface({ sessionId }: ChatInterfaceProps) {
  ...
  const [showMode, setShowMode] = useState<'both' | 'thinking' | 'doing'>('both');
                ...
                {msg.plan && (
                  <div className="mb-4 space-y-3">
                    ...
                    <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-md">
                      <div className="text-xs font-semibold text-blue-400 mb-1">Understanding</div>
                    ...
                    {/* Thinking */}
                    {msg.plan.thinking && (showMode === 'both' || showMode === 'thinking') && (
                      ...
                      <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-md">
                          ...
                          {msg.plan.steps.map((step, idx) => (
                            <div key={idx} className="flex items-start gap-2 text-xs">
                              ...
                              <div>
                                ...
                                {step.reasoning && (
                                  <div className="text-muted-foreground ml-4 mt-0.5">↳ {step.reasoning}</div>
                                )}
                              </div>
                            </div>
                          ))}
                        ...
                        {msg.plan.requires_visualization && (
                          <div className="mt-2 text-xs text-green-400 flex items-center gap-1">
                            <BarChart2 size={12} />
                            <span>Visualization: {msg.plan.visualization_type || 'auto'}</span>
                          </div>
                        )}
                      </div>
                    )}
      ...
      <div className="p-4 bg-background/80 backdrop-blur-sm border-t border-border sticky bottom-0">
        ...
        <div className="flex items-center gap-3 mb-3 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={showMode}
              onChange={(e) => setShowMode(e.target.value as 'both' | 'thinking' | 'doing')}
